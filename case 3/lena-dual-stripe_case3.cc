          ...
          if (enbdev)
            {
              ...
              outFile << "set label \"" << enbdev->GetCellId ()
                      << "\" at "<< pos.x << "," << pos.y
                      << " left font \"Helvetica,4\" textcolor rgb \"white\" front  point pt 2 ps 0.3 lc rgb \"white\" offset "<<(enbdev->GetCellId()-1)/19 <<",0" //andrei - added cellId printout instea of "0"
                      << std::endl;
            }
        }
    }
...
main (int argc, char *argv[])
{
  ...
  if (nMacroEnbSites > 0)
    {
      ...
      macroUeBox = Box (-1250 * 1732/500,
                        1250 * 1732/500,
                        -1100 * 1732/500,
                        2200*areaMarginFactor * 1732/500,//areaMarginFactor is 0.5
                        ueZ, ueZ);
    }
  else
    {
  ...
  //Andrei - Uncomment half of next line for clustered user distribution
  uint32_t nClusterUes = 90;//90;//round (macroUeAreaSize * macroUeDensity) / 3;
  NodeContainer clusterUes;
  clusterUes.Create(nClusterUes);
  ...
  //this is calculated in km so needs to be converted from meters => 128.1 - 37.6*3
...
//   LogComponentEnable ("PfFfMacScheduler", LOG_LEVEL_ALL);
  ...
  std::cout<<"\nTPower: "<<macroEnbTxPowerDbm<<std::endl;
  lteHelper->SetEnbAntennaModelType ("ns3::ParabolicAntennaModel");
  lteHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (70));
  lteHelper->SetEnbAntennaModelAttribute ("MaxAttenuation",     DoubleValue (25.0));
  lteHelper->SetEnbAntennaModelAttribute ("Orientation",     DoubleValue (6.0));//15.0 for case 1 3D
  ...
  std::cout<<"Alg "<<lteHelper->GetFfrAlgorithmType()<<"\n";

  //std::cout<<"RSR Threshold "<<lteHelper->GetFfrAlgorithmType()<<"\n";
  //std::cout<<"Alg "<<lteHelper->m_phyStats->LteStatsCalculator<<"\n";

  NetDeviceContainer macroEnbDevs = lteHexGridEnbTopologyHelper->SetPositionAndInstallEnbDevice (macroEnbs);

  //double m_d = interSiteDistance;
  //int m_gridWidth = 2500;
  //double m_yMin = -1250;
  //double _xMin = -1250;
  //double m_offset = 0.5;
  double _siteHeight = 32;

  ///////begin andrei
  double _x[19] = {-750, -1000, -750, -500, -250, -500, -250, -500, 0, 250, 0, 250, 0, 500, 750, 500, 750, 500, 1000};
  double _y[19] = {-433.012701892219,0,433.012701892219,-866.025403784439,-433.012701892219,0,433.012701892219,866.025403784439,
  				   -866.025403784439,-433.012701892219,0,433.012701892219,866.025403784439,-866.025403784439,-433.012701892219,
  				    0,433.012701892219,866.025403784439,0};
    //MobilityHelper mobility;
  //mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  ...
  for (int i=0; i<19; i++)
  {
	  enbPositionAlloc->Add (Vector (_x[i] * 1732/500, _y[i] * 1732/500, _siteHeight));                       // eNB positions
  }
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility.SetPositionAllocator (enbPositionAlloc);
  mobility.Install (macroEnbs);
  ...
  if (outdoorUeMaxSpeed!=0.0)
    {
      ...
      positionAlloc = CreateObject<RandomDiscPositionAllocator> ();


      positionAlloc->SetAttribute ("X", DoubleValue (macroUeBox.xMin*1/3));
      positionAlloc->SetAttribute ("Y", DoubleValue (macroUeBox.yMin*1/3));


      positionAlloc->SetAttribute ("Rho", StringValue("ns3::UniformRandomVariable[Min=0.0|Max=1000.0]"));

      mobility.SetPositionAllocator(positionAlloc);
      mobility.Install (clusterUes);

      for (NodeContainer::Iterator it = clusterUes.Begin ();
                       it != clusterUes.End ();
                       ++it)
                    {
                      (*it)->Initialize ();
  ...
  BuildingsHelper::Install (clusterUes);

  NetDeviceContainer macroUeDevs = lteHelper->InstallUeDevice (macroUes);
  NetDeviceContainer clusterUeDevs = lteHelper->InstallUeDevice (clusterUes);
  ...
  if (epc)
    {
      ...
      lteHelper->Attach (clusterUeDevs);
  ...
  else
    {
      ...
      lteHelper->AttachToClosestEnb (clusterUeDevs, macroEnbDevs);
  ...
  if (epc)
    {
      ...
      Ptr<ExponentialRandomVariable> staTime = CreateObject<ExponentialRandomVariable> ();
                  ...
                  if (epcDl)
                    {
                      ...
                      //std::cout<<"Using TCP for EPC DL\n";
                      //Andrei - override start time for poisson arrival rate 0.2
                      //Andrei - also need to make a loop to send several files(time dependent)
                      //uint32_t noFiles = 0;

                      //Andrei - need mean arrival time in seconds for ns3 exponential variable usage
                      // meanArrival = 1/(poisson_arrival_rate) = 1/0.2 = 5.0
                      double meanArrival = 5.0;
                      double beginTime = 0;
                      int noFiles = 0;

                      //Andrei - allocate as many applications as needed per UEs, depending on simulation time available
                      ...
                      while(beginTime<=simTime)
                      {
                    	  ...
                    	  dlClientHelper.SetAttribute ("MaxBytes", UintegerValue (2000000));

                    	  clientApps.Add (dlClientHelper.Install (remoteHost));
                    	  beginTime = beginTime + startTimeSeconds->GetValue () + staTime->GetValue(meanArrival,0);
                    	  std::cout<< "Round: " << noFiles << "/" << uint32_t(simTime / meanArrival) << " Time: " << beginTime << " Port: "<< dlPort << "\n";
                    	  clientApps.Start( Seconds ( beginTime ) );
                      ...
                      //myApp.Start(Seconds (startTimeSeconds->GetValue () + staTime.GetValue()));
