...
main (int argc, char *argv[])
{
  ...
  //Andrei: overriding number of bearers per UE to accommodate for FTP file transfer (2MB)
  ...
  if (nMacroEnbSites > 0)
    {
      ...
      macroUeBox = Box (-1250,
                        1250,
                        -1100,
                        2200*areaMarginFactor,//areaMarginFactor is 0.5
                        ueZ, ueZ);
    }
  else
    {
  ...
  //Andrei - Used custom pathloss model required by 3GPP 36814-900 document
  //Pathloss = 128.1 + 37.6*log10(R), where R in km
  //Formula is calculated for km so needs to be converted to meters for NS3, which gives:
  //15.3 + 37.6*log10(r), where r in m
...
//   LogComponentEnable ("PfFfMacScheduler", LOG_LEVEL_ALL);
  ...
  lteHelper->SetEnbAntennaModelAttribute ("Orientation",     DoubleValue (15.0));//15.0 for case 1 3D
  ...
  ///////begin andrei
  //Setting location of cells in hexagonal pattern, two rings - 19 cells
  ...
  for (int i=0; i<19; i++)
  {
	  enbPositionAlloc->Add (Vector (_x[i], _y[i], _siteHeight));                       // eNB positions
  }
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility.SetPositionAllocator (enbPositionAlloc);
  mobility.Install (macroEnbs);
  ...
  if (epc)
    {
      ...
      //Andrei: using a log file
      std::ofstream ofs;
      ofs.open("/home/andrei/workspace/Huawei/mylog.txt", std::fstream::out| std::fstream::trunc);
          ...
          for (uint32_t b = 0; b < numBearersPerUe; ++b)
            {
              ...
              //Andrei: commented following line due to poisson timing in TCP->epcDl case
